...
export async function selectProfilePlantByProfileId(profilePlant: ProfilePlant) {
    try {
        ...
        const query : string = 'SELECT profilePlantPlantId, profilePlantProfileId, profilePlantNotes FROM profilePlant WHERE profilePlantProfileId = UUID_TO_BIN(:profilePlantProfileId);'
